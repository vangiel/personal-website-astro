...
From an ML perspective, when factoring in an approximately equal time commitment and effort when generating answers, providing a single scalar for each scenario would yield answers for a higher number of scenarios.
This would, in turn, generate a higher variability in the input scenarios that would make the model less prone to overfitting.
...
For each scenario in the bootstrapped dataset a matrix of 73 x 73 samples is generated.
...
This process follows the same steps as ([SNGNN](/navigation/sngnn)), with the exception that there is an additional grid of 18 x 18 nodes whose values are passed to the CNN layers of the architecture and decoded into the final output.
