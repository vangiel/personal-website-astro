...
pubDate: "Jul 08 2022"
heroImage: "@/images/blog/1_MLP/neuron_perceptron.png"
imageAlt: "Diagram of a neuron and a perceptron"
tags: ["AI", "Basic concept"]
slug: "mlp"
---

import MLPSchemeImg from "@/images/blog/1_MLP/MLP_scheme.png";
...
<p class="caption"> **Fig 1.** _Multi-Layer Perceptron scheme._ </p>
...
Moreover, the result of **Eq. 1** is passed through a non-linear operation **$\sigma(\cdot)$**, also called **activation function**, that modules the intensity and range of the values at the output, thereby enabling the MLP to approximate non-linear functions.
...
**Fig. 1** shows how several Perceptrons are connected to form the Multi-Layer Perceptron.
...
Conventionally, these layers are separated into three parts as shown in **Fig 1**: the input layer, the hidden layers and the output layers.
...
import depthVSAccuracy from "@/images/blog/1_MLP/depthvsaccuracy.png";

<Image src={depthVSAccuracy} alt="Graph showing the effect of network depth on accuracy" />
<p class="caption"> **Fig 2.** _Relation between the number of layers and accuracy of a model trained to transcribe multi-digit numbers from photographs of addresses by [Deep Learning book](https://www.deeplearningbook.org/)_ </p>
...
The input layer (most left-hand side in **Fig. 1**) takes the data to be processed by the network.
We can express the input values of the network as a vector **$\vec{x} = (x_{0}, x_{1} ... x_{m})^{T}$**. 
We can now transform the equation of the computation of a single Perceptron (**Eq. 1**) in its vectorised version:

$$z^{(l)}_{i} = \vec{w}^{(l)T}_{i}\vec{x} + b^{(l)}_{i} $$ (2)

$$a^{(l)}_{i} = \sigma(z^{(l)}_{i})$$ (3)
...
$$\vec{z}^{(l)} = W^{(l)}\vec{x} + \vec{b}^{(l)}$$ (4)

$$\vec{a^{(l)}} = \sigma(\vec{z}^{(l)})$$ (5)
...
Finally, if we want to do a forward pass through a dataset with **$m$** samples we can write **Eq. 1** as:
